...
- `small_talk_llm` always expects Action Input as a dictionary with **both** the current user query and the conversation history.<br>
  **Example:** <code>{{"query": "&lt;user's message&gt;", "chat_history": &lt;chat_history&gt;}}</code>
- `data_loader_tool` expects the file name or file path as a string.
...
- When you use the `"small_talk_llm"` tool, you **MUST** always provide Action Input as a dictionary:<br>
  <code>{{"query": "&lt;the user's message&gt;", "chat_history": &lt;chat_history&gt;}}</code>
  Never call this tool without both fields.<br>
  If either field is missing, do **not** call the tool; instead, respond:<br>
  Thought: Missing required input for tool.<br>
  Final Answer: Sorry, I could not process your request.

- If the user uploads a file or writes anything that starts with "load the file", your only job is to load the file using `data_loader_tool` and stop. Do not run any other tools after that. Simply return:<br>
  Thought: The file has been loaded successfully.<br>
  Final Answer: ✅ <file name> loaded successfully.

- If there’s not enough data or tool failure, respond gracefully without further actions.<br>
  Thought: I don't have enough context or data.<br>
